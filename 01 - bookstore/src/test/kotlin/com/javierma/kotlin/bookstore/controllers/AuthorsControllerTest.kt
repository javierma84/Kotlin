...
import com.javierma.kotlin.bookstore.testAuthorDtoA
import com.javierma.kotlin.bookstore.testAuthorEntityA
import com.javierma.kotlin.bookstore.testAuthorUpdateRequestDtoA
...
class AuthorsControllerTest @Autowired constructor(
    ...
    fun `test that create Author returns HTTP 400 when IllegalArgumentException is thrown`() {
        ...
        mockMvc.post(AUTHORS_BASE_URL) {
            ...
            content = objectMapper.writeValueAsString(
                com.javierma.kotlin.bookstore.testAuthorDtoA()
            )
        }.andExpect {
            status { isBadRequest() }
        }
    ...
    @Test
    fun `test that get returns HTTP 404 when author not found in the database`() {
        every {
